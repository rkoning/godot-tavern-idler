...
| 2026-07-13 | `/requirement` — build convention (no REQ): adapter tickets own their own `src/adapters/<x>/<x>.csproj`; test project auto-discovers domain+adapter projects via MSBuild glob, so no ticket edits `.sln`/`Tests.csproj`. New TKT-027 owns the one-time build change | glob auto-discovery / per-ticket sln edits / single-owner scaffolding | user |
| 2026-07-13 | `/requirement` — CON-009 → v1.1 staff-content validation (no new REQ; refines REQ-060/063/064/095): wage ≥ 0, ≥1 trait per staffer, paidService price ≥ 0, required fields non-empty, unique ids, `namedHire.role` exists, trait ids exist in catalog (cross-file), unknown fields fail-fast. TKT-028 carries the conformance-suite update. **Follow-up:** extend the cross-file trait-existence check to rooms (CON-003) + guests (CON-005) later | staff-only now + follow-up / extend to rooms+guests now | user |
