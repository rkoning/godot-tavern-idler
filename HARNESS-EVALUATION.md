...
## HEV-006 — Parallel dispatch fans out before ticket requirements are validated, so headless agents repeatedly hit missing/incomplete requirements
- **Date:** 2026-07-15
- **Stage/command involved:** implementation dispatch (parallel-wave dispatcher launching headless `/implement` sessions)
- **Severity:** friction (recurring)
- **Problem:** The wave dispatcher selects eligible tickets and immediately fans out headless agents, with no requirement-validation step in between. Tickets that looked eligible turn out to have missing or incomplete requirements, ambiguous acceptance criteria, or unresolved issues — and the headless agents, having no human to ask, repeatedly stall, guess, or set themselves BLOCKED on gaps that a human could have resolved in seconds up front. The result is a high rate of wasted parallel runs and rework. Ambiguity is discovered *inside* each isolated session (the worst place to discover it) rather than at the dispatch boundary where a human is present.
- **Expected:** Each wave should run a **planning + validation pass before any fan-out**: for every candidate ticket, produce a plan of all changes it entails, surface every missing/incomplete requirement and open issue, and resolve them **interactively with the user**. Acceptance criteria may be edited during this pass **only with explicit user consent** (never silently, and never inside a headless session). Only tickets that survive validation clean get dispatched as headless parallel agents; the rest are held for the user to fix or route through `/requirement`.
- **Proposed change:** Insert a pre-dispatch planning/validation stage into `.claude/commands/dispatch-wave.md` (between wave selection §2 and worktree prep §3): per-ticket change plan → raise all requirement gaps/issues → interactive resolution with the user → gated, consent-only acceptance-criteria edits → then fan out only the validated, clean tickets. Complements HEV-002 (which restricts *which* ticket types dispatch autonomously) by validating the *readiness* of the ones that do.
- **Status:** OPEN

## HEV-007 — Dispatched sessions aren't wave-aware, and there is no automated wave merge; integration is manual and error-prone
- **Date:** 2026-07-16
- **Stage/command involved:** parallel-wave dispatch + merge-back (the dispatcher launches headless per-ticket sessions; merge is left to a hand-run reminder)
- **Severity:** friction (recurring, near-blocker at integration time)
- **Problem:** Two coupled gaps. (a) **No wave-awareness in the workers.** A dispatched per-ticket session is told to work in *its* worktree/branch and commit, but nothing tells it that it is one of many sibling sessions in a coordinated wave, or that its branch is a wave artifact rather than the final integration target. When such a session reaches the end of its work it treats merging as its own responsibility, tries to reconcile against a moving shared branch, and stalls or misbehaves — because from inside the isolated worktree it has no model of the wave it belongs to. (b) **No automated wave merge.** Integration is only a printed reminder telling the human to `git checkout main; git merge tkt-### …` for each branch by hand, resolve board conflicts, and clean up. The human has to know exactly which branches belong to the wave and merge them one at a time — the branches aren't tagged as a wave, so there's no way to enumerate "everything in wave N" programmatically, and no reviewing pass sits between the raw per-ticket branches and main.
- **Expected:** (a) Every dispatched worker should be explicitly told it is part of a parallelized wave and that its **only** integration job is to finish its ticket and commit to its own branch — never to merge into or reconcile against a shared/base branch. Its branch should carry a **wave tag/marker** (naming convention or git tag) so the whole wave is enumerable later. (b) A dedicated **merge-orchestration session** (run on the most capable model, since it reviews and reconciles real code) should: enumerate every branch tagged for wave N, review each one (including the existing governance/ownership sweep), merge them together into a single integration branch named for the wave (e.g. `wave-<N>-<short-description>`), resolve cross-branch conflicts (board rows, shared append-only logs) once, and push *that* branch to the remote as the reviewed unit to be merged into main — with main merge/push still gated on the user.
- **Proposed change:** In `.claude/commands/dispatch-wave.md`: (1) tag each worker branch to its wave (branch-name convention already encodes the ticket; add a wave marker — e.g. `git tag wave-<N> tkt-###` or a `wave-<N>/tkt-###` branch namespace — so `git branch --list 'wave-<N>/*'` / `git tag --list 'wave-<N>*'` enumerates the wave). (2) Add to the §5 dispatch addendum an explicit statement: "You are one worker in parallel wave N; commit to your branch only; do NOT merge, rebase onto, or reconcile against any other branch — integration is handled by a separate session." (3) Replace the §6 hand-run merge reminder with a new **`/harness:merge-wave <N>`** command (or a documented spawn of a headless Opus session) that enumerates the wave's tagged branches, runs the HEV-004 governance sweep per branch, merges them into `wave-<N>-<description>`, resolves conflicts once, and pushes that branch — leaving only the main-merge for the user. Ties together HEV-004 (governance sweep belongs inside this merge step) and HEV-005 (worktree/branch naming + cleanup).
- **Status:** OPEN
